...
module "google_network" {
  ...
  service_project_service_accounts = keys({ for account in module.google_project : account.service_account => account if account.folder_id == module.google_folder[each.value.parent_folder_name].folder_name && account.project_id != module.google_project[each.value.host_project_identifier].project_id })
